...
*/
Route::get('/', function () {
    $videos = VideoEdukasi::latest()->get()->chunk(4);
    $artikelTerbaru = Artikel::latest()->take(4)->get();

    return view('welcome', compact('videos', 'artikelTerbaru'));
})->name('home');

Route::get('/artikel', [AdminArtikelController::class, 'publicIndex'])
    ->name('artikel');

Route::get('/artikel/{id}', [AdminArtikelController::class, 'publicShow'])
    ->name('artikel.show');

Route::middleware('guest')->group(function () {
    Route::get('login', [AuthController::class, 'showLoginForm'])->name('login');
    Route::post('login', [AuthController::class, 'login'])->name('login.post');
    Route::get('/artikel', [AdminArtikelController::class, 'publicIndex'])->name('artikel');
    Route::get('/artikel/{id}', [AdminArtikelController::class, 'publicShow'])->name('artikel.show');
});
...
Route::middleware(['auth', 'checkRole:admin'])->prefix('admin')->group(function () {
    ...
    Route::resource('/data-video', AdminVideoController::class)->names('admin.video');
    ...
    Route::get('/tarik-saldo/saldo/{nasabahId}', [AdminTarikSaldoController::class, 'getMetodePencairan'])->name('admin.tarik-saldo.saldo');
